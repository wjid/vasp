...
#        for key, val in Vasp.default_parameters.iteritems():   # Changed for Python 3 compat
        ...
        for symbol in symbols:
            ...
            for i, atom in enumerate(atoms):
                if atom.symbol == symbol and i not in sort_indices:
                    ...
                    count += 1
            if count > 0:
                # Patch to always use Ti_sv
                if symbol == 'Ti':
                    ppp += [[symbol,
                             'potpaw_{}/{}/POTCAR'.format(pp, 'Ti_sv'),
                             count]]                
                else:
                    ppp += [[symbol,
...
#         s_p = {k:round(v,5) if isinstance(v,float) else v for k,v in self.parameters.iteritems()}
        f_p = {k:round(v,5) if isinstance(v,float) else v for k,v in file_params.items()}
        s_p = {k:round(v,5) if isinstance(v,float) else v for k,v in self.parameters.items()}
        
#         if not {k: v for k, v in s_p.iteritems()
        if not {k: v for k, v in s_p.items()
            ...
            if os.path.exists(self.outcar):
                with open(self.outcar) as f:
                    ...
                    if 'Voluntary context switches:' not in lines[-1]:
                        # This takes care of the case where vasp is compiled with a profiler
                        #   Added by Hines on 7/25/17; increased on 7/4/19
                        myEnd = min(len(lines), 800)
                        for k in range(2, myEnd):
                            if 'Voluntary context switches:' in lines[-k]:
                                return Vasp.FINISHED
...
#        return json.loads(encode(d))       # Python 3 compat
        return json.loads(encode(list(d)))
